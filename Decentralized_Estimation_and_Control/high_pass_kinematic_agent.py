...
class HighPassKinematicAgent:
    def __init__(self, agent_id, agents=None):
        ...
        Controls with velocity instead of acceleration.
    ...
    def set_initial_state(self, pos, vel):
        ...
        '''
        self.state = np.array([[pos[0], pos[1]]]).T
    ...
    def F(self, state):
        ...
        ui = - np.linalg.inv(self.B + self.Jphi(pi).T @ self.Lambda @ self.Jphi(pi))  @  self.Jphi(pi).T @ self.GAM @ (yi - self.f_star)
