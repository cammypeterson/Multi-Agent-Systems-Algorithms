...
This file contains the ILQSolver class for solving a time-varying,
finite horizon LQ game using the iLQ algorithm.
"""
...
class ILQSolver(object):
    """
    Solves a time-varying, finite horizon LQ game using the iLQ algorithm.
    """

    def __init__(self, dynamics, player_costs, Ps, alphas, alpha_scaling=0.05,
        ...
        - player_costs (list of Cost): list of player costs.
        - Ps (list of ArrayImpl): list of P matrices.
        - alphas (list of ArrayImpl): list of alpha vectors.
        - alpha_scaling (float): scaling factor for alpha vectors.
        - max_iter (int): maximum number of iterations.
        - u_constraints (list of ArrayImpl): list of control constraints.
    ...
    def reset(self, Ps_warmstart=None, alphas_warmstart=None):
        ...
        Args:
        - Ps_warmstart (list of ArrayImpl): warmstart for Ps.
        - alphas_warmstart (list of ArrayImpl): warmstart for alphas.
        """
    ...
    def run(self, x0, Ps_warmstart=None, alphas_warmstart=None):
        ...
        while (iteration <= self._max_iter):
            ...
            # Compute the current operating point and performs line search.
            tt = time.time()
            ...
            # Linearize about this operating point.
            tt = time.time()
            ...
            # Compute feedback Nash equilibrium of the resulting LQ game.
            tt = time.time()
            As = np.asarray(As)
            ...
            # Accumulate total costs for all players.
    ...
    def _compute_operating_point(self, current_xs: ArrayImpl, current_us: list, Ps: list,
                                 alphas: list, x0: ArrayImpl) -> Tuple[ArrayImpl, list]:
        """
        ...
        # Compute the joint state trajectory.
        ...
        for k in range(self._horizon):
            ...
            # Compute the control policy for each player.
            us_k = []
            for ii in range(self._num_players):
                # Compute the feedback strategy.
            ...
            # Compute the next state for the joint system.
    ...
    @partial(jit, static_argnums=(0,))
    def _linearize_dynamics(self, xs: ArrayImpl, us_list: list,
        ...
        for k in range(self._horizon):
            A, B = self._dynamics.linearize_discrete_jitted(xs[:, k], [us_list[ii][:, k]
                for ii in range(self._num_players)], k, args
            )
            As = As.at[:, :, k].set(A)
    ...
    def _quadraticize_costs(self, xs: ArrayImpl,
                            us_list: list) -> Tuple[list, list, list, list]:
        """
    ...
    def _solve_lq_game(self, As: np.ndarray, Bs_list: list, Qs_list: list, ls_list: list,
                       Rs_list: list) -> Tuple[list, list, list, list]:
        """
        ...
        # Recursively compute all intermediate and final variables.
        ...
        for k in range(horizon - 1, -1, -1):
            ...
            # Update zetas to be the next step earlier in time.
            for ii in range(num_players):
                zetas[ii][:, k] = F.T @ (zeta[ii] + Z[ii] @ beta) \
                    + l[ii] + P_split[ii].T @ R[ii] @ alpha_split[ii]
